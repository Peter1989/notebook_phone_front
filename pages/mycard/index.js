...
Page({
  ...
  fetchData(page) {
    if (this.data.nomore || !this.data.levelOneArr ||this.data.levelOneArr.length === 0) {
      return
    }
    var tag = '';
    ...
    wx.request({
      url: 'https://www.funyang.top/minipro/minipro/mycard',
      data: {
        page: page,
        size: that.data.size,
      ...
      success: function (res) {
        ...
        res.data.list.map((item) => {
          item.datetime = app.formatStamp(item['updatetime'] * 1000, '/');
          //用于标识显示和删除操作互斥的的变量
          item.open = false;
          item.show = false;
          item.answer_text = [{
  ...
  renewtags: function(){
    var that = this;
    if (!this.data.levelOneArr || that.data.levelOneArr.length === 0) {
      return;
    }
  ...
  handleshow: function (e) {
    ...
    if(cardList1[index].open === true){
      cardList1[index].open = false;
      that.setData({ cardList: cardList1 });
      return;
    }
  ...
  touchM: function (e) {
    ...
    if (e.touches.length == 1) {
      ...
      //如果正在显示答案，不能移动
      console.log('show',list[index].show)
